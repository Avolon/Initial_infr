...
resource "yandex_iam_service_account" "roman-skillfactory" {
  name = "roman-skillfactory"
}

# Назначаем права созданного аккаунта
resource "yandex_resourcemanager_folder_iam_member" "roman-skillfactory-admin" {
  folder_id = var.yandex_folder_id
  role = "admin"
  member = "serviceAccount:${yandex_iam_service_account.roman-skillfactory.id}"
  depends_on = [
    yandex_iam_service_account.roman-skillfactory,
  ]
}

# Создаем ключи доступа Static Access Keys
resource "yandex_iam_service_account_static_access_key" "static-access-key" {
  service_account_id = yandex_iam_service_account.roman-skillfactory.id
  depends_on = [
    yandex_iam_service_account.roman-skillfactory,
  ]
}
...
# Ключ в данном случае подбираем уже готовый с нашего ПК по адресу: /home/avolon/.ssh/
  metadata = {
